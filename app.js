...
async function fetchNewCoins() {
    coinList.innerHTML = '<div style="text-align:center; padding:50px; color:#14f195;">🔍 Mengekstrak Nama Koin Asli...</div>';
...
function displayCoins(items) {
    ...
    items.slice(0, 15).forEach(item => {
        ...
        // --- LOGIKA EKSTRAKSI NAMA DARI URL (PASTI MUNCUL) ---
        // Contoh URL: https://dexscreener.com/solana/4k3dy...pump
        // Kita ambil bagian terakhir setelah slash
        let rawName = item.url.split('/').pop().toUpperCase();
        let displayName = rawName.length > 15 ? "NEW TOKEN" : rawName;
